...
<div class="card-body">
    <div class="text-left">
        <a class="btn btn-light"  :href="'/vendor/create'">
...
<!--    <a :href="'/'">444</a>-->
    <table class="table table-striped table-hover">
        ...
        <tbody>
        <tr class="text-center" v-for="(vendor,key) in laravelData.data">
            <td v-if="page ==1" >
                {{ key + 1}}
            </td>
            <td v-else>
            ...
            <td >{{vendor.name}}</td>
            <td >{{date(vendor.created_at)}}</td>
            <td>
                <a :href="'/vendor/'+vendor.id + '/edit'" data-toggle="tooltip"
                   data-title="ویرایش">
                ...
                <a @click="deleted(vendor.id)" class="deleteRecord">
...
export default {
    ...
    methods:{
        getData(page = 1){
            this.page = page;
            axios.get('/api/get-vendor?page=' + page)
                .then(res =>{
